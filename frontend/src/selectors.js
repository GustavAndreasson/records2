...
export const selectCollection = createSelector(
  ...
  (collection, artistCollection, viewArtistCollection) =>
...
export const selectOrderedFilteredCollection = createSelector(
  ...
  (
    ...
    return orders
      ...
      .reduce((col, rec) => (filterRecord(rec) ? col.concat(rec) : col), [])
  }
)
...
export const selectCollectionStats = createSelector(
  ...
  (orderedFilteredCollection, rate) => {
    ...
    let priceSum =
      rate *
      orderedFilteredCollection.reduce(
        (sum, rec) => sum + (rec.price ? parseFloat(rec.price) : 0),
        0
      )
    return {
...
export const selectDirectLink = createSelector(
  ...
  state => state.artist.activeArtist,
  state => state.artist.viewArtistCollection,
  (discogsUsername, orders, filters, activeArtist, viewArtistCollection) =>
    Persistant.query({
